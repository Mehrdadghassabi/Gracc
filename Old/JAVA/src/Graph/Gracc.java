 ...
 */
public final class Gracc {
    private final Graph graph;
    private final Branch[] branches;
    private Map<Integer,Byte> holder=new HashMap<>();
    boolean CallForsecond=false;
    public  ArrayList<ArrayList<Direction>> Loops;
    int Calling_count=0;

    public Gracc(Graph graph, ArrayList<ArrayList<ElectricalGate>> in) throws GeneratorException, SingletonException{
            
    this.graph=graph;
    this.branches=Get_Branches(graph,in);
    ...
    public ArrayList<ArrayList<Direction>> getLoop() throws GeneratorException, SingletonException{
     ...
     for(int i=0;i<this.graph.Get_Unknown_Number()-this.graph.Get_Nodes_Number()+1;i++){
    ...
    private Branch[] Get_Branches(Graph g,ArrayList<ArrayList<ElectricalGate>> in){
    Branch[] reblocks=new Branch[g.Get_Unknown_Number()];
    ...
    Node[][] branchNodes=new Node[2][g.Get_Unknown_Number()];
    ...
    for(Branch block:this.branches){
    ...
    int[] nums=this.graph.DFT_Get_Loops().get(x);
    ...
    public Direction getBranchXmorethan2(int first, int last){
     byte hold=(holder.get(JoinTwonum(first,last))==null)?0:
             ((byte)(holder.get(JoinTwonum(first,last))+1));
     if(this.CallForsecond)
         hold--;
         
     holder.put(JoinTwonum(first,last), hold);
     int in=holder.get(JoinTwonum(first,last));
